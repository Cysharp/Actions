...
public sealed class ScanPrUnicodeCommand(IPrChangeSource? changeSource = null)
{
        ...
        ScanState state)
    {
        ...
        foreach (var rune in text.EnumerateRunes())
        {
            ...
            if (IsFormat(value))
                ...
                reason = "non-ASCII space in a file name";
            ...
            else if (value is 0x0085 or 0x2028 or 0x2029)
            {
                line++;
                column = 1;
                previousWasCarriageReturn = false;
            ...
            {
                column += rune.Utf16SequenceLength;
                previousWasCarriageReturn = false;
            }
        }
    }
    ...
    internal static string FormatAnnotation(UnicodeViolation violation)
    {
        ...
        if (violation.Line > 0)
        {
            var messageWithLocation = $"{source}:{violation.Line}:{violation.Column}: {message}";
            return $"::error file={EscapeProperty(source)},line={violation.Line},col={violation.Column},title=Forbidden Unicode::{EscapeData(messageWithLocation)}";
        }
